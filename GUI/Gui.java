	...
	public void start(Stage primaryStage) {
		...
		try {
           ...
           Level1.setOnAction(event -> {
			...
			imGoat.setOnMouseClicked(event -> {

				System.out.println("xxxxxxxx");

				translateTransition.setNode(imGoat);

				imGoat.setX(imGoat.getX() + 200);

				translateTransition.setByY(0);
				p.toboat(crosserArrayList.get(0),imGoat.getX());

				//translateTransition.play();

			});
			imPlant.setOnMouseClicked(event -> {

				System.out.println("xxxxxxxx");

				translateTransition.setNode(imPlant);

				imPlant.setX(imPlant.getX() + 200);

				translateTransition.setByY(0);
				p.toboat(crosserArrayList.get(0),imPlant.getX());

				//translateTransition.play();

			});
			imWolf.setOnMouseClicked(event -> {

				System.out.println("xxxxxxxx");

				translateTransition.setNode(imWolf);

				imWolf.setX(imWolf.getX() + 200);

				translateTransition.setByY(0);
				p.toboat(crosserArrayList.get(0),imWolf.getX());
